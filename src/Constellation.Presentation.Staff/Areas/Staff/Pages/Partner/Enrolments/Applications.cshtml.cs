...
public class ApplicationsModel : BasePageModel
{
    ...
    public async Task<IActionResult> OnGet()
    {
            ...
            && Guid.TryParse(raw, out var guid))
        {
            ...
            if (result.IsSuccess)
                return RedirectToPage("/Partner/Enrolments/Applications/Index", new { area = "Staff", PeriodId = periodId });
        }
        ...
        if (periodsResult.Value.Count == 1)
            return RedirectToPage("/Partner/Enrolments/Applications/Index", new { area = "Staff", PeriodId = periodsResult.Value.First().Id });
