...
public class UpdateUserSchoolsClaimCommandHandler : IRequestHandler<UpdateUserSchoolsClaimCommand>
{
    ...
    public UpdateUserSchoolsClaimCommandHandler(UserManager<AppUser> userManager, IAppDbContext context, IActiveDirectoryGateway gateway)
    {
    ...
    public async Task<Unit> Handle(UpdateUserSchoolsClaimCommand request, CancellationToken cancellationToken)
    {
        ...
        var adSchools = await _gateway.GetLinkedSchoolsFromActiveDirectory(request.EmailAddress);
