...
internal sealed class CoverStartDateChangedDomainEvent_SendCoverUpdatedEmailHandle
    ...
    public async Task Handle(CoverStartDateChangedDomainEvent notification, CancellationToken cancellationToken)
    {
        ...
        foreach (var teacher in teachers)
        {
            if (primaryRecipients.All(entry => entry.Email != teacher.EmailAddress))
            {
                ...
                {
                    _logger.Warning("{action}: Could not create valid email address for {teacher} during processing of cover {id}", nameof(CoverEndDateChangedDomainEvent_SendCoverUpdatedEmailHandler), teacher.DisplayName, cover.Id);
        ...
        {
            _logger.Error("{action}: Could not create valid email address for covering teacher during processing of cover {id}", nameof(CoverEndDateChangedDomainEvent_SendCoverUpdatedEmailHandler), cover.Id);
