...
using Constellation.Application.Domains.SciencePracs.Queries.GetLessonRollDetailsForSchoolsPortal;
    ...
    : IQueryHandler<GetLessonRollSubmitContextForSchoolsPortalQuery, ScienceLessonRollForSubmit>
{
    ...
    public async Task<Result<ScienceLessonRollForSubmit>> Handle(GetLessonRollSubmitContextForSchoolsPortalQuery request, CancellationToken cancellationToken)
    {
        ...
        if (lesson is null)
        {
            ...
            return Result.Failure<ScienceLessonRollForSubmit>(SciencePracLessonErrors.NotFound(request.LessonId));
        }
        ...
        if (roll is null)
        {
            ...
            return Result.Failure<ScienceLessonRollForSubmit>(SciencePracRollErrors.NotFound(request.RollId));
        }
        ...
        if (roll.Status == Core.Enums.LessonStatus.Completed)
        {
            if (!string.IsNullOrWhiteSpace(roll.SubmittedBy))
            {
                ...
                if (roll.SubmittedBy.Contains('@'))
                {
                    ...
                    {
                        _logger
                            .ForContext(nameof(GetLessonRollSubmitContextForSchoolsPortalQuery), request, true)
                            .ForContext(nameof(Error), emailAddress.Error, true)
                            .Warning("Failed to retrieve Lesson Roll details");

                        return Result.Failure<ScienceLessonRollForSubmit>(emailAddress.Error);
                    }
