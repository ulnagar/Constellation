...
using Constellation.Application.Domains.EnrolmentContext.Offers.Commands.MarkOfferDocumentsCollected;
...
internal sealed class MarkOfferReviewCompletedCommandHandler
    : ICommandHandler<MarkOfferReviewCompletedCommand>
{
    private readonly IEnrolmentOfferRepository _offerRepository;
    private readonly IEnrolmentUnitOfWork _unitOfWork;
    private readonly ILogger _logger;

    public MarkOfferReviewCompletedCommandHandler(
        ...
        ILogger logger)
    {
        ...
        _logger = logger
            .ForContext<MarkOfferReviewCompletedCommand>();
    }

    public async Task<Result> Handle(MarkOfferReviewCompletedCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            _logger
                .ForContext(nameof(MarkOfferReviewCompletedCommand), request, true)
                .ForContext(nameof(Error), EnrolmentOfferErrors.NotFound(request.OfferId), true)
                .Warning("Failed to mark Offer as Review Completed");

            return Result.Failure(EnrolmentOfferErrors.NotFound(request.OfferId));
        }

        Result result = offer.MarkReviewComplete(request.Username);
