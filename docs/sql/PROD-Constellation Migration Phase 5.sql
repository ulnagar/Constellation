...
SELECT Id, Title, UrlPath, OfferingId, ShowLink FROM [ACOS].[dbo].OFFERINGRESOURCES
WHERE 1 = 1;
...
SELECT Lesson_Id, CourseOffering_Id FROM [ACOS].[dbo].LessonCourseOfferings
WHERE 1 = 1;
...
SELECT Id, Type, StudentId, OfferingId, Date AS datetime2, PeriodName, PeriodTimeframe, AbsenceLength, AbsenceTimeframe, AbsenceReason, StartTime, EndTime, ExternallyExplained, ExternalExplanation, ExternalExplanationSource, DateScanned AS datetime2, LastSeen AS datetiem2 FROM [ACOS].[dbo].[ABSENCES]
WHERE 1 = 1;
...
SELECT Id, OfferingId, StartDate AS datetime2, EndDate AS datetime2, IsDeleted, UserType, CasualId, StaffId FROM [ACOS].[dbo].[CLASSCOVERS]
WHERE 1 = 1;
...
SELECT Id, StudentId, OfferingId, IsDeleted, DateCreated AS datetime2, DateDeleted AS datetime2 FROM [ACOS].[dbo].[ENROLMENTS]
WHERE 1 = 1;
...
SELECT Id, OfferingId, StaffId, PeriodId, RoomId, IsDeleted, DateCreated AS datetime2, DateDeleted AS datetime2 FROM [ACOS].[dbo].OfferingSessions
WHERE 1 = 1;
